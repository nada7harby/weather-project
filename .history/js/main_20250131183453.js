...
const citiesUrl = "https://countriesnow.space/api/v0.1/countries"; 
const selectCity = document.getElementById("city");
const countryLinks = document.querySelectorAll('.item a'); // جلب جميع الأعلام
...
fetch(citiesUrl)
  .then(res => res.json())
  .then(data => {
    countriesData = data.data; // تخزين بيانات الدول
  })
  .catch(err => console.log("هناك خطأ: ", err));

// تحديث قائمة المدن في select
function updateCities(countryName) {
  selectCity.innerHTML = `<option>Select City</option>`; // مسح القائمة القديمة
...
// إضافة الحدث لكل علم دولة
countryLinks.forEach(link => {
  link.addEventListener('click', (e) => {
    e.preventDefault(); // منع التنقل للرابط
    const countryName = link.getAttribute('data-country'); // جلب اسم الدولة
    updateCities(countryName);


    // الانتقال للسكشن المستهدف
    // تنفيذ السكرول للـ forecast
    document.getElementById("forecast").scrollIntoView({ behavior: "smooth" });
  });
});

// عند اختيار مدينة جديدة، يتم جلب الطقس الخاص بها
selectCity.addEventListener("change", (event) => {
